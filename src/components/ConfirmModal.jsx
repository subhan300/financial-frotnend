                ...
                >
                    <div className='flex items-center justify-center flex-shrink-0 h-24 w-24 rounded-xl bg-red-100 text-red-500 mx-auto'>
                        <svg className="w-12 h-12" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 2L2 21h20L12 2zm0 4v4m0 4v4"></path>
                        </svg>
                    </div>
                    <h1 className="text-xl text-center mt-5 font-bold text-slate-800 dark:text-slate-100">Are you sure?</h1>
                    <p className='text-sm text-center mx-auto justify-center max-w-sm mt-3 text-gray-500'>This action cannot be undone, All values associated with this field will be lost.</p>
                    <button onClick={() => {
                                handleDeleteExpense(value);
                              }} className='w-full py-2 bg-[#4F46E5] hover:bg-[#433BCB] text-white rounded-md mt-5'>Delete</button>
                    <button onClick={() => setModalOpen(false)} className='w-full py-2 border border-gray-700 rounded-md mt-2'>Cancel</button>
                </div>
            </Transition>
        </>
    );
